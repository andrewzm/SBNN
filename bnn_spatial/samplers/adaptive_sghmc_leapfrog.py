"""
Sampler for scale-adapted (adaptive) SGHMC, with leapfrog discretisation
"""
...
class AdaptiveSGHMC(Optimizer):
    def __init__(self, params, lr=1e-2, num_burn_in_steps=3000, epsilon=1e-16, mdecay=0.05, scale_grad=1.):
        """
        Adaptive SGHMC sampler with one leapfrog step.
    ...
    def step(self, closure=None):
        ...
        #     print('Parameter tensor {} has shape {}'.format(param_idx, list(parameter.shape)))

        # Iterate over tensors containing weights/biases for each layer, initially given by net.parameters()
        for param_idx, parameter in enumerate(group["params"]):

            # If parameter tensor has no gradient, skip it (all parameter tensors have requires_grad=True)
            ...
            # Preconditioner
            M_inv = 1. / (torch.sqrt(v_hat) + epsilon)

            # Variance of noise see Eq 10 in [1] right, the Gaussian term
            epsilon_var = (0.5 * (lr ** 2) * mdecay * M_inv - 0.25 * (lr ** 4))
            ...
            momentum.add_(-0.5 * (lr ** 2) * M_inv * gradient - 0.5 * mdecay * momentum + sample_t)

            # Update parameters (Eq 10 left in [1])
            parameter.detach().add_(momentum)

            # Update momentum again (using leapfrog method)
            gradient = parameter.grad.detach() * scale_grad
            momentum.add_(-0.5 * (lr ** 2) * M_inv * gradient - 0.5 * mdecay * momentum + sample_t)
