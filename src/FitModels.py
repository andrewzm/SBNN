...
if(config.get('EMBEDDING_LAYER') == True):
    ...
    hidden_dims = [sqrt_emb_size**2, 40, 40, 40]  # list of hidden layer dimensions
else:
...
if(config.get('DO_STAGE2') == True):
    ...
    np.save(OUT_DIR + '/Target_posterior_samples.npy', gp_preds_)

    # Plot 4-by-4 panels of samples
    plot_samples_2d(samples=gp_preds_,
                    extent=[-margin_h, margin_h, -margin_v, margin_v])
    plt.savefig(FIG_DIR + '/Target_posterior_samples.png', bbox_inches='tight')
    ...
    for ckpt in mcmc_checkpoints:
        ...
        np.save(OUT_DIR + '/BNN_posterior_samples.npy', bnn_optim_preds_)
        ...
        if ckpt == num_ckpt:
            ...
            plot_output_traces(domain=test_tensor,
                            ...
                            n_burn=n_burn_thin,
                            legend_entries=legend_entries)
            plt.savefig(FIG_DIR + '/BNN_posterior_trace.png', bbox_inches='tight')

            # Histogram of network output at same input point(s)
            plot_output_hist(domain=test_tensor, preds=bnn_optim_preds)
            plt.savefig(FIG_DIR + '/BNN_posterior_hist.png', bbox_inches='tight')
            ...
            plot_output_acf(domain=test_tensor,
                            ...
                            n_chains=n_chains,
                            n_samples_kept=n_samples)
            plt.savefig(FIG_DIR + '/BNN_posterior_acf.png', bbox_inches='tight')
        ...
        if (len(mcmc_checkpoints) > 1) & (ckpt < num_ckpt):
            ...
            plot_mean_sd(mean_grid=opt_pred_mean,
                        ...
                        domain=test_tensor,
                        obs=X)
            plt.savefig(FIG_DIR + '/BNN_posterior_step{}.png'.format(int(50*ckpt)), bbox_inches='tight')

        # Plot 4-by-4 panels of samples
        plot_samples_2d(samples=bnn_optim_preds_,
                        extent=[-margin_h, margin_h, -margin_v, margin_v])
        plt.savefig(FIG_DIR + '/BNN_posterior_samples_step{}.png'.format(int(50 * ckpt)), bbox_inches='tight')
    ...
    plt.savefig(FIG_DIR + '/Target_posterior.png', bbox_inches='tight')

    np.save(OUT_DIR + '/Target_pred_mean.npy', gp_pred_mean)
    np.save(OUT_DIR + '/Target_pred_sd.npy', gp_pred_sd)
    ...
    plot_mean_sd(mean_grid=opt_pred_mean,
                ...
                sd_range=[sd_min, sd_max],
                mean_range=[mean_min, mean_max])
    plt.savefig(FIG_DIR + '/BNN_posterior.png', bbox_inches='tight')

    np.save(OUT_DIR + '/BNN_pred_mean.npy', opt_pred_mean)
    np.save(OUT_DIR + '/BNN_pred_sd.npy', opt_pred_sd)
